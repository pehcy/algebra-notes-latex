...
\end{theorem}
\begin{proof}
    Given that $a, b \in H$ and that $b^{-1}$ is in $H$. We have $ab^{-1} \in H$ and 
    so the One-step subgroup test is satisfied.
\end{proof}
...
\end{theorem}
\begin{proof}
    We need to show that $a^{-1}$ is in $H$, and then using the Two-step subgroup test to verify.

    For an $a \in H$, if $a = e$ then $a^{-1} = e^{-1} = e$ and we are done. However, 
    if $a \neq e$, consider another subgroup $A = \{a, a^2, a^3, \ldots \} \subseteq H$ by closure. 
    Since $H$ is finite, two of these must be identical, say
    \[
        a^j = a^k \quad \text{for } 1 \leq j < k.
    \]
    Now cancelling $a^j$ on both sides, we get
    \begin{align*}
        e = a^{k-j} &\implies ea^{-1} = a^{k-j}\, a^{-1}\\
        &\implies a^{-1} = a^{k-j-1} \in A.
    \end{align*}
    Hence $a^{k-j-1}$ is the inverse of $a$ and is an element of $A$, which also in $H$. By Two-step subgroup
    test, the finite subgroup test is valid.
\end{proof}
...
\begin{definition}[Generating subgroup]
    ...
    \begin{equation}
        \langle a \rangle = \{ \underbrace{a \cdot a \cdot \cdots \cdot a}_{n \text{ times}} \> | \> n \in \mathbb{Z} \} = \{ a^n \> | \> n \in \mathbb{Z} \}
    \end{equation}

    If the group is abelian and is additive, then 
    \begin{equation}
        \langle a \rangle = \{ \underbrace{a + a + \cdots + a}_{n \text{ times}} \> | \> n \in \mathbb{Z} \}= \{ na \> | \> n \in \mathbb{Z} \}
    \end{equation}
\end{definition}
...
\begin{definition}[Center of group]
    The \textbf{center}, $Z(G)$\footnote[1]{The $Z$ stands for ``Zentrum``, a German word for ``Center``}, of a group $G$ is a subset of elements in $G$ that commute with every element of $G$, that is, 
    \begin{equation}
...
For $n \geq 3$, the dihedral group $D_{2n}$ is described as the rigid motions taking a regular 
...
\begin{figure}[ht]
    ...
    \caption{The rotation $r$ on dihedral group $D_6$ with order 6. $r$ in $D_6$ is described as rotating equilateral triangle $120$ degree.}
\end{figure}

\begin{figure}[ht]
    \centering
    \makebox[\textwidth]{\includegraphics[width=0.8\paperwidth]{./images/Dihedral2.pdf}}
    \caption{The group action $f$ on dihedral group $D_6$ with order 6. $f$ is a horizontal flip.}
\end{figure}
...
\begin{theorem}
    Let the $2n$-degree dihedral group  
    \[
...
\begin{example}
    In dihedral group order 7, simplify $r^2sr^6sr^3$ and $sr^4sr^3sr^2$.
\end{example}
\begin{solution}
    In $D_7$, compute 
    \begin{align*}
        r^2sr^6sr^3 &= r^2(sr^6)sr^3 & \text{Associativity}\\
        &= r^2(r^{-6}s)sr^3 & \text{Since } sr^{6} = r^{-6}s\\
        &= r^2r^{-6}ssr^3\\
        &= r^{-4}r^3 & s^2 = e, \text{flip two time horizontally will back to original}.\\
        &= r^{-1} & \text{Simplifying}\\
        &= r^6 & \text{1 anticlockwise rotation} = \text{6 counterclockwise rotation}.
    \end{align*}

    On the other hand, 
    \begin{align*}
        sr^4sr^3sr^2 &= s(r^4s)r^3(sr^2) & \text{Associativity}\\
        &= s(sr^{-4})r^3(r^{-2}s) & \text{Since } r^4s = sr^{-4}, sr^{2} = r^{-2}s\\
        &= ssr^{-4}r^3r^{-2}s\\
        &= r^{-3}s & s^2 = e, \text{flip two time horizontally will back to original}.\\
        &= r^4s & \text{3 anticlockwise rotation} = \text{4 counterclockwise rotation}.
    \end{align*}
\end{solution}
...
\begin{example}
    ...
    \begin{align*}
        ...
        &1 + H = \{1 + 0, 1 + 3, 1 + 6\} \> (\text{mod} \, 9) = \{1, 4, 7\}\\
        &2 + H = \{2 + 0, 2 + 3, 2 + 6\} \> (\text{mod} \, 9) = \{2, 5, 8\}\\
        &3 + H = \{3 + 0, 3 + 3, 3 + 6\} \> (\text{mod} \, 9) = \{3, 6, 0\} = H
    \end{align*}
    ...
    \begin{align*}
        ...
        &H + 1 = \{0 + 1, 3 + 1, 6 + 1\} \> (\text{mod} \, 9) = \{1, 4, 7\}\\
        &H + 2 = \{0 + 2, 3 + 2, 6 + 2\} \> (\text{mod} \, 9) = \{2, 5, 8\}\\
        &H + 3 = \{0 + 3, 3 + 3, 6 + 3\} \> (\text{mod} \, 9) = \{3, 6, 0\} = H
    \end{align*}
...
\begin{example}
    Prove that if $G$ is abelian, then every subgroup $H$ is normal.
\end{example}
\begin{solution}
    Taking arbitrary $g \in G$. If $G$ is abelian, then 
    \[
...
\begin{algorithm}[ht]
    ...
    \Function{find\_element\_with\_max\_order($G$: FiniteGroup) $\to$ Array<tuple(element, order)>}{
        \boxit{codegreen}{2}
        $orders \gets \var{compute\_orders}(G)$\;
...
\begin{mdframed}
    ...
    \begin{Answer}
        \begin{enumerate}
            \item Yes
            \item Yes
            \item No, because $-1, 2 \in \mathbb{R}^*$ but $\sqrt{-1 \times 2} \notin \mathbb{R}^*$.
            \item No, because there is no inverse.
        \end{enumerate}
    \end{Answer}
    ...
    \begin{Answer}
        \begin{align*}
            &a * b * c = e\\
            &\Rightarrow a^{-1} * a * b * c = a^{-1} * e\\
            &\Rightarrow e * b * c = a^{-1}\\
            &\Rightarrow b * c * a = a^{-1} * a\\
            &\Rightarrow b * c * a = e
        \end{align*}
    \end{Answer}
    ...
    \end{Exercise}
    \begin{Answer}
        For all $a,b$ in $G$, since every element is its own inverse in $G$. Hence
        \begin{align*}
            ab = (ab)^{-1} &\implies ab = b^{-1} a^{-1} & \text{socks-shoes theorem}\\
            &\implies ab = ba & \text{since } a = a^{-1} \text{ and } b = b^{-1}\\
        \end{align*}
    \end{Answer}
    ...
    \vspace{0.752cm}
    \begin{Exercise}
        Which of the following groups are cyclic? List all the generators of the group.
        \begin{enumerate}
            \item $G_1 = \{ 6^n \> | \> n \in \mathbb{Z} \}$ under multiplication.
            \item $G_2 = \{ a + b\sqrt{2} \> | \> a,b \in \mathbb{Z} \}$ under addition.
        \end{enumerate}
    \end{Exercise}
    ...
    \vspace{0.752cm}
    \begin{Exercise}
        Let $G$ be a group and and $a \in G$ generates a cyclic group of order 2 and is 
        the unique such element. Show that $ax=xa$ for all $x \in G$.
    \end{Exercise}
    ...
    \vspace{0.752cm}
    \begin{Exercise}
        Show that $\mathbb{Z}_p$ has no proper nontrivial subgroup if $p$ is prime.
    \end{Exercise}

    \vspace{0.752cm}
    \begin{Exercise}
        Let $G$ be an abelian group and $H$ and $K$ are finite subgroups of $G$ such that 
        $|H| = r$ and $|K| = s$. Show that if $gcd(r,s) = 1$, then $G$ contains a cyclic subgroup of order $rs$.
    \end{Exercise}

    \vspace{0.752cm}
    \begin{Exercise}
        Let $G$ be a group and $a \in G$. Show that the centralizer of $a$ in $G$,
        \[
            C(a) = \{ x \in G \> | \> xa = ax \},
        \]
        is a subgroup of $G$.
    ...
    \vspace{0.752cm}
    \begin{Exercise}
        Find all cosets of the subgroup $\langle 3\rangle$ of $\mathbb{Z}_{24}$.
    \end{Exercise}
    ...
    \end{Exercise}
    \begin{Answer}
        Check that $\phi(x+y) = \phi(x)\phi(y)$.
        \[
            \phi(x+y) = 5^{x+y} = 5^x \cdot 5^y = \phi(x) \phi(y)
        \]
        which preserves the group operation. 
        \begin{enumerate}
            \item (One-to-one) Suppose $\phi(x) = \phi(y)$, 
            \begin{align*}
                \phi(x) = \phi(y) &\Rightarrow 5^x = 5^y \\
                &\Rightarrow \log_5 5^x = \log_5 5^y \\
                &\Rightarrow x = y.
            \end{align*}

            \item (Onto) For all $x, y \in G$, we have 
            \[
                \phi(x) = y \Rightarrow 5^x = y \Rightarrow x = \log_5 y. \forall y > 0
            \]
    ...
    \vspace{0.752cm}
    \begin{Exercise}
        Let $\phi : G \to G'$ be a group homorphism and let $N \triangleleft G$. 
        Show that $\phi(N) \triangleleft \phi(G)$.
    \end{Exercise}

    \vspace{0.752cm}
    \begin{Exercise}
        Let $\phi : G \to G'$ be a group homorphism and let $N' \triangleleft G'$. 
        Show that $\phi^{-1}(N') \triangleleft G$.
    \end{Exercise}
    ...
    \end{Exercise}
    \begin{Answer}
        Since $\phi$ is a homomorphism, then for all $n \in \mathbb{Z}_{10}$, we have 
        \[
            \phi(n) = \phi(\underbrace{1 + 1 + \cdots + 1}_{n \text{ times}}) = n \cdot \phi(1) = 8n \mod 20.
        \]
        Thus, 
        \begin{align*}
            ker(\phi) &= \{ n \in \mathbb{Z}_{10} \> | \> \phi(n) = 0 \}\\
            &= \{ n \in \mathbb{Z}_{10} \> | \> 8n \equiv 0 \mod 20 \}\\
            &= \{ n \in \mathbb{Z}_{10} \> | \> n = 0, 5\}\\
            &= \{ 0, 5\}
        \end{align*}
    \end{Answer}
    ...
    \vspace{0.752cm}
    \begin{Exercise}
        Find the conjugacy classes of dihedral group order 8. 
    \end{Exercise}
    \begin{Answer}
        The dihedral group of order 8 is 
        \[
            D_8 = \{ e, r, r^2, r^3, s, sr, sr^2, sr^3 \}
        \]
        The members of a conjugacy class of $D_8$ are different but have the 
        same type of effect on a 2D square: $r$ and $r^3$ are a 90 degree rotation 
        in clockwise and counter-clockwise respectively; $s$ and $sr^2$ are reflections 
        across a diagonal; and $sr$ and $sr^3$ are reflections across an edge bisector.
        \[
            \{ e \}
            \{ r, r^3 \},
            \{ r^2 \},
    ...
    \vspace{0.752cm}
    \begin{Exercise}
        Let $G = S_4$ and let the Klein-four group
        \[
    ...
    \end{Exercise}
    \begin{Answer}
        Suppose $N$ is a normal subgroup of $S_4$. Since $|S_4| = 24$, then by Lagrange's theorem,
        the possible orders of $N$ are $1, 2, 3, 4, 6, 8, 12, 24$. If $|N| = 1$, then $N = \{ e \}$.
        And If $|N| = 24$, then $N = S_4$.
        
        Now suppose $1 < |N| < 24$. We know that $N$ is a union of conjugacy 
        classes including $Cl(e)$. The conjugacy classes, as described as above, have 
        size $1, 6, 8, 6$ and $3$ respectively. It is possible to have $|N| = 2, 3$. Thus $|N|$ is even.

        Since $Cl(e) \subseteq N$, we must also have $Cl((1 \> 2)(3 \> 4)) \subseteq N$ 
        in order to have $|N|$ be even. Notice that 
        \[
            H = Cl(e) \cup Cl((1 \> 2)(3 \> 4)) \leq S_4.
        \]
        Therefore, $H$ is a normal subgroup of $S_4$.

        If $N$ includes one of the conjugacy classes of size $6$, then 
        $|N|$ cannot be a proper divisor of $24$, as it would have to be 
        \[
        1 + 3 + 6 = 10, \quad 1 + 3 + 6 + 8 = 18, \quad 1 + 3 + 6 + 6 = 16.
        \]
        It is possible for $N$ to contain the conjugacy class of size $8$ because 
        $1 + 3 + 8 = 12$. In fact,
        \[
            A_4 = Cl(e) \cup Cl((1 \> 2 \> 3)) \cup Cl((1 \> 2)(3 \> 4))
        \]
        is a normal subgroup of $S_4$ with order $12$.
    \end{Answer}
    ...
    \end{Exercise}
    \begin{Answer}
        \textit{For the sake of contradiction}, suppose $G$ is non-abelian group of order $p^2$, 
        then $Z(G) < G$; Since $Z(G)$ is nontrivial ($Z(G) \neq \{e\}$), then $|Z(G)| = p$. 
        By Lagrange's theorem, $|G/Z(G)| = p^2/p = p$. Since $G/Z(G)$ is of prime order, then $G/Z(G)$ is cyclic.
        By theorem, if $G/Z(G)$ is cyclic, then $G$ is abelian. This contradicts our assumption that $G$ is non-abelian.
    ...
    \vspace{0.752cm}
    \begin{Exercise}
        If $H \leq G$ such that the product of two right cosets of $H$ is always a right coset of $H$ in $G$,
        prove that $H \triangleleft G$.
    \end{Exercise}
    
    \vspace{0.752cm}
    \begin{Exercise}
        Show that if $H$ is a subgroup of index 2 in a finite group $G$, then every left coset of $H$ is also a 
        right coset of $H$.
    \end{Exercise}
    ...
    \end{Exercise}
    \begin{Answer}
        Let $H \cap K = M$. We want to show $M$ is a subgroup of $G$. Applying one-step subgroup test:
        \begin{enumerate}
            \item Since $H$ and $K$ are subgroups of $G$, then $e \in H$ and $e \in K$. Thus, $e \in M$. $M$ is nonempty set. 
            \item Let $a, b \in M$. Then $a, b \in H$ and $a, b \in K$. Since $H$ and $K$ are subgroups of $G$, then $ab^{-1} \in H$ and $ab^{-1} \in K$. Therefore, $ab^{-1} \in M$. 
        \end{enumerate}
        Therefore, by one-step subgroup test, $M \leq G$.
    \end{Answer}
